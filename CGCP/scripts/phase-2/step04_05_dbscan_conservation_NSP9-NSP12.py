#!/usr/bin/env python3
"""
CGCP Phase 2 Steps 4+5 - DBSCAN + Conservation Overlay: NSP9-NSP12
Combined script for efficiency.
Anchor: ARG733 (NSP12, cons=1.000)

Run:
    cd ~/projects/rtc-pan-coronavirus
    python3 CGCP/scripts/phase-2/step04_05_dbscan_conservation_NSP9-NSP12.py
"""
...
OUT4    = os.path.join(BASE,
    'CGCP/02-deep-dive/NSP9-NSP12/step-04-clusters')
...
def save_step4(records, labels, cluster_info):
    ...
    tsv = os.path.join(OUT4, 'clusters_NSP9-NSP12.tsv')
    ...
    json_out = {
        ...
        'n_clusters':  int(len([l for l in cluster_info
                                if l >= 0])),
        'n_noise':     int(sum(1 for l in labels
                               if l == -1)),
        'clusters':    {str(k): v
                        for k,v in cluster_info.items()},
        'residues':    rows,
    }
    jpath = os.path.join(OUT4, 'clusters_NSP9-NSP12.json')
...
def save_step5(records, labels):
    ...
    for r in records:
        result.append({
            **r,
            'cons_tier': get_tier(r['conservation']),
        })
    ...
    jpath = os.path.join(OUT5,
        'conservation_overlay_NSP9-NSP12.json')
    with open(jpath, 'w') as f:
        json.dump({
            'interface':  'NSP9-NSP12',
            'anchor':     'ARG733',
...
def make_figures(records, labels, cluster_info,
    ...
    panel_title(ax1, 'A',
                'DBSCAN clusters (X-Z projection)',
                f'eps={EPS}Å | ★=ARG733 anchor')
    ...
    p1 = os.path.join(OUT4,
        'Fig_Step04_DBSCAN_NSP9-NSP12.png')
    fig1.savefig(p1)
    plt.close()
    ...
    # Panel A: conservation per residue (top 30)
    ...
    # Panel C: composite vs conservation
    for r in result:
        ec = TIER_COLORS[r['cons_tier']]
        fc = TIER_FILLS[r['cons_tier']]
        is_anc = r['primary_feature'] == 'anchor'
    ...
    p2 = os.path.join(OUT5,
        'Fig_Step05_ConservationOverlay_NSP9-NSP12.png')
    fig2.savefig(p2)
    plt.close()
    print(f"  Fig Conservation: {p2}")


def print_summary(cluster_info, labels, result,
                  tier_stats):
    print('\n' + '='*65)
    print('STEPS 4+5: DBSCAN + CONSERVATION — NSP9-NSP12')
    print('='*65)
    ...
    for r in candidates[:12]:
        ...
        print(f"  {sym} {r['aa']}{r['position']:<10} "
              f"{r['chain']:<7} "
...
if __name__ == '__main__':
    print('CGCP Phase 2 Steps 4+5 — NSP9-NSP12')
    ...
    print(f"\nStep4 outputs: {OUT4}")
    print(f"Step5 outputs: {OUT5}")
    print("Next: Step 6 — integrated assessment")
