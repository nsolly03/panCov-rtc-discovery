#!/usr/bin/env python3
"""
CGCP Phase 2 Steps 4+5 - DBSCAN + Conservation Overlay: NSP7-NSP8
Same method as all previous interfaces.
Anchor: PHE92 (NSP8, Chain A, cons=0.800)

Run:
    cd ~/projects/rtc-pan-coronavirus
    python3 CGCP/scripts/phase-2/step04_05_dbscan_conservation_NSP7-NSP8.py
"""
...
S3_DIR = os.path.join(BASE,
    'CGCP/02-deep-dive/NSP7-NSP8/step-03-features')
OUT4   = os.path.join(BASE,
    'CGCP/02-deep-dive/NSP7-NSP8/step-04-clusters')
...
def load_features():
    path = os.path.join(S3_DIR,
        'feature_classification_NSP7-NSP8.tsv')
    records = []
    with open(path) as f:
...
def save_step4(records, labels, cluster_info):
    ...
    tsv = os.path.join(OUT4, 'clusters_NSP7-NSP8.tsv')
    ...
    jpath = os.path.join(OUT4, 'clusters_NSP7-NSP8.json')
    ...
    for tier in ['identical','high','moderate','variable']:
        ...
        if members:
            tier_stats[tier] = {
                ...
                'members':   [r['residue'] for r in members],
                'nsp8':      [r['residue'] for r in members
                              if r['chain']=='NSP8'],
                'nsp7':      [r['residue'] for r in members
                              if r['chain']=='NSP7'],
            }

    jpath = os.path.join(OUT5,
        'conservation_overlay_NSP7-NSP8.json')
    with open(jpath, 'w') as f:
        json.dump({
            'interface':  'NSP7-NSP8',
            'anchor':     'PHE92',
...
def make_figures(records, labels, cluster_info,
    ...
    p1 = os.path.join(OUT4,
        'Fig_Step04_DBSCAN_NSP7-NSP8.png')
    ...
    top = sorted(result, key=lambda x: -x['composite'])[:35]
    ...
    ax_a.set_xticks(x)
    set_xticklabels_vertical(ax_a, labels_a, fontsize=6.5)
    ...
    for r in result:
        ...
        if is_anc or (r['conservation'] >= 0.800
                      and r['composite'] >= 0.750):
            ax_c.annotate(
                r['residue'].split('-')[1],
    ...
    p2 = os.path.join(OUT5,
        'Fig_Step05_ConservationOverlay_NSP7-NSP8.png')
...
def print_summary(cluster_info, labels, result, tier_stats):
    print('\n'+'='*65)
    print('STEPS 4+5: DBSCAN + CONSERVATION — NSP7-NSP8')
    print('='*65)
    ...
    print(f"\n  Pharmacophore preview "
          f"(cons>=0.800, comp>=0.500): {len(candidates)}")
...
if __name__ == '__main__':
    print('CGCP Phase 2 Steps 4+5 — NSP7-NSP8')
