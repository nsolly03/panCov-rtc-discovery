#!/usr/bin/env python3
"""
CGCP Phase 2 Steps 6+7 - Assessment + Pharmacophore: NSP7-NSP8
Same method as all previous interfaces.
Anchor: PHE92 (NSP8, Chain A, cons=0.800)

Pharmacophoric elements:
  E1 — Aromatic/hydrophobic core (NSP8)
       PHE92(anchor), LEU98, MET94, ARG111
  E2 — NSP7 contact zone
       SER57, LEU56, MET75
  E3 — Extended NSP8 contacts
       PRO116, GLN88, ARG96

Run:
    cd ~/projects/rtc-pan-coronavirus
    python3 CGCP/scripts/phase-2/step06_07_assessment_pharmacophore_NSP7-NSP8.py
"""
...
S5_DIR   = os.path.join(BASE,
    'CGCP/02-deep-dive/NSP7-NSP8/step-05-conservation')
OUT6     = os.path.join(BASE,
    'CGCP/02-deep-dive/NSP7-NSP8/step-06-assessment')
OUT7     = os.path.join(BASE,
    'CGCP/02-deep-dive/NSP7-NSP8/step-07-pharmacophore')
...
EL_NAMES  = {
    'E1': 'E1 — Aromatic/hydrophobic core (NSP8)',
    'E2': 'E2 — NSP7 contact zone',
    'E3': 'E3 — Extended NSP8 contacts',
}


def load_conservation():
    path = os.path.join(S5_DIR,
        'conservation_overlay_NSP7-NSP8.tsv')
    records = []
    with open(path) as f:
...
def assess(records):
    ...
    for row in records:
        ...
        if pos == ANCHOR_RES and chain == 'NSP8':
            decision = 'ANCHOR'
...
def save_step6(assessed):
    tsv = os.path.join(OUT6,
        'integrated_assessment_NSP7-NSP8.tsv')
    fields = ['residue','chain','position','aa',
              'primary_feature','conservation',
              'cons_tier','composite',
              'e1_cluster0','e2_nsp8','e3_cons_high',
    ...
    jpath = os.path.join(OUT6,
        'integrated_assessment_NSP7-NSP8.json')
...
def get_coords(assessed):
    ...
    chain_a   = structure['A']  # NSP8
    chain_b   = structure['B']  # NSP7

    coords = {}
    for r in assessed:
        chain_obj = chain_a if r['chain']=='NSP8' else chain_b
        pos       = r['position']
        try:
            res = chain_obj[(' ', pos, ' ')]
            ca  = res['CA'].coord
...
def assign_elements(assessed, coords):
    ...
    for r in assessed:
        ...
        if r['decision'] == 'ANCHOR':
            el = 'E1'
        elif chain == 'NSP8' and pos in (98,94,111):
            el = 'E1'
        elif chain == 'NSP7':
            el = 'E2'
        else:
        ...
        pharm_type = (
            ...
            'UNKNOWN')
        tol = (1.0 if pharm_type=='ANCHOR'
               else 1.5 if pharm_type in ('AROMATIC',
                                          'CHARGED_POS',
                                          'CHARGED_NEG')
               else 2.0)
...
def save_step7(features, centroids):
    ...
    pharmacophore = {
        ...
        'features':   features,
        'screening_notes': (
            'Compounds targeting CGCP-NSP7-NSP8-v1 should: '
            '(1) present aromatic/hydrophobic scaffold for E1 '
            '(PHE92/LEU98/MET94 core); '
            '(2) engage NSP7 contact zone via H-bond/polar '
            'groups (SER57/LEU56 — E2); '
            '(3) optionally reach E3 extended contacts. '
            'Note: lowest pan-coronavirus conservation (0.660) — '
            'selectivity for SARS-CoV-2 may be higher than '
            'pan-coronavirus activity. Verify with MERS/NL63 sequences.'
        ),
    }

    jpath = os.path.join(OUT7,'pharmacophore_NSP7-NSP8.json')
    with open(jpath,'w') as f:
        json.dump(pharmacophore,f,indent=2,cls=NpEncoder)
    print(f"  Step7 JSON: {jpath}")

    tsv = os.path.join(OUT7,'pharmacophore_NSP7-NSP8.tsv')
...
def make_figures(assessed, out6, pharmacophore, centroids):
    ...
    top = [r for r in assessed if r['decision']!='EXCLUDE'][:30]
    x   = np.arange(len(top))
    ...
    ax1.text(len(top)-0.5,4.08,'INCLUDE (4/6)',
             fontsize=7,color=P['include'],ha='right',clip_on=False)
    ax1.text(len(top)-0.5,3.08,'SECONDARY (3/6)',
             fontsize=7,color=P['secondary'],ha='right',clip_on=False)
    ...
    for f in features:
        ...
        ax_a.scatter(f['x'],f['z'],color=fill,
                     ...
                     s=size,marker=marker,alpha=0.88,zorder=4)
    ...
    make_legend(ax_a,[
        ...
        for el in ['E1','E2','E3'] if el in centroids
    ...
    p2 = os.path.join(OUT7,
        'Fig_Step07_Pharmacophore_NSP7-NSP8.png')
...
def print_summary(assessed, out6, pharmacophore, centroids):
    ...
    print(f"\n  Pharmacophore candidates "
          f"({len(out6['pharmacophore_candidates'])}):")
    ...
    print(f"\n  Note: cons=0.660 (lowest interface) — "
          f"compound selectivity may favor SARS-CoV-2.")
    print('='*65)


if __name__ == '__main__':
    print('CGCP Phase 2 Steps 6+7 — NSP7-NSP8')
